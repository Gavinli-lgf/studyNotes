...
* **线性映射的矩阵**
* 线性映射T要想量化，那么V、W的基要是确定的，才能用标量a<sub>j,k</sub>的举行阵列，把映射关系T给量化。（书面语：T关于基（v<sub>1</sub> , ... , v<sub>n</sub>）和基（w<sub>1</sub> , ... , w<sub>n</sub>）的矩阵，基为M(T,(v<sub>1</sub> , ... , v<sub>n</sub>),(w<sub>1</sub> , ... , w<sub>n</sub>))。）
